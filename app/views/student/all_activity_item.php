...
<?php 
    ...
    $isQuiz = (isset($assignment['type']) && $assignment['type'] == 'quiz');
    $barColor = $isQuiz ? 'bg-purple-500' : 'bg-amber-500';
    $iconBg = $isQuiz ? 'bg-purple-50 text-purple-600 border-purple-200' : 'bg-amber-50 text-amber-600 border-amber-200';
    $iconClass = $isQuiz ? 'fa-puzzle-piece' : 'fa-gamepad';
    $hoverBorder = $isQuiz ? 'group-hover:border-purple-300' : 'group-hover:border-amber-300';
...
<div class="hunt-card group relative overflow-hidden <?php echo $hoverBorder; ?>">
    <div class="absolute left-0 top-0 bottom-0 w-1.5 <?php echo $barColor; ?>"></div>
        ...
        <div class="flex flex-col md:flex-row md:justify-between md:items-start gap-4">
            ...
            <div>
                <h3 class="text-lg font-bold text-neutral-900 leading-tight group-hover:text-primary-700 transition-colors mb-2">
                    <a href="<?php echo site_url('/courses/show/' . $assignment['course_id'] . '?tab=activities'); ?>" class="hover:underline">
            ...
            <div class="flex flex-col sm:items-end gap-2 mt-2 md:mt-0">
                ...
                <a href="<?php echo site_url(($isQuiz ? '/quiz/' : '/assignment/') . $assignment['assignment_id']); ?>" class="btn btn-sm w-full sm:w-auto justify-center bg-white border border-neutral-200 text-neutral-600 font-semibold hover:text-primary-700 hover:border-primary-300 shadow-sm transition-all rounded-lg px-4 py-2">
                    <?php echo $assignment['submission_id'] ? 'View Submission' : ($isQuiz ? 'Take Quiz' : 'Submit Work'); ?>
                </a>
            </div>
        </div>
    </div>
